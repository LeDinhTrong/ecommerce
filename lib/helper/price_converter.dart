...
class PriceConverter {
  static String convertPrice(BuildContext context, double price,
    ...
    return '${(price * Provider.of<SplashProvider>(context, listen: false).myCurrency.exchangeRate * (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency.exchangeRate)).toStringAsFixed(0).replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}${Provider.of<SplashProvider>(context, listen: false).myCurrency.symbol}';
  }
  ...
  static double convertPriceTMoMo(BuildContext context, double price,
      {double discount, String discountType}) {
    if (discount != null && discountType != null) {
      if (discountType == 'amount' || discountType == 'flat') {
        price = price - discount;
