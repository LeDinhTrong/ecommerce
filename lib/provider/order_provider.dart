...
class OrderProvider with ChangeNotifier {
  ...
  List<OrderModel> get deliveredList =>
      _deliveredList != null ? _deliveredList.reversed.toList() : _deliveredList;
  ...
  Future<void> initOrderList(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      apiResponse.response.data.forEach((order) {
        ...
        db.findById(orderModel.id.toString()).then((value) {
          if (value != null && value != 'null') {
            orderModel.orderHash = value;
            notifyListeners();
          }
        });
  ...
  void getOrderDetails(String orderID, BuildContext context, String languageCode) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      apiResponse.response.data
          .forEach((order) => _orderDetails.add(OrderDetailsModel.fromJson(order)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  Future<void> placeOrder(
      OrderPlaceModel orderPlaceModel, Function callback, List<CartModel> cartList) async {
    _isLoading = true;
    notifyListeners();
    print('orderPlaceModel.toJson() ${orderPlaceModel.toJson()}');
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      //insert to mongo
      orderPlaceModel.orderId = orderID;
      final db = MongoDatabase();
      db.find().then((value) {
        db.insert(
          info: orderPlaceModel.toJson(),
          previousHash: value.last['_id'],
        );
      });
  ...
  void setProvinceId(int index) {
    _provinceId = index;
    notifyListeners();
  }

  void setDistrictId(int index) {
    _districtId = index;
    notifyListeners();
  }

  void setWardId(int index) {
    _wardId = index.toString();
    notifyListeners();
  }
  ...
  Future<void> initShippingList(BuildContext context) async {
    ...
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
      ...
      apiResponse.response.data.forEach(
          (shippingMethod) => _shippingList.add(ShippingMethodModel.fromJson(shippingMethod)));
    } else {
      ApiChecker.checkApi(context, apiResponse);
    }
    notifyListeners();
  ...
  Future<void> initTrackingInfo(
      String orderID, OrderModel orderModel, bool fromDetails, BuildContext context) async {
    if (fromDetails) {
      _orderDetails = null;
    }
    if (orderModel == null) {
