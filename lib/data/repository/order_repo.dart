...
class OrderRepo {
  ...
  Future<ApiResponse> getOrderList() async {
    try {
      final response =
          await dioClient.get(AppConstants.BASE_URL + AppConstants.ORDER_URI);
  ...
  Future<ApiResponse> getOrderDetails(
    ...
    try {
      final response = await dioClient.get(
        AppConstants.BASE_URL + AppConstants.ORDER_DETAILS_URI + orderID,
  ...
  Future<ApiResponse> getShippingList() async {
    try {
      final response = await dioClient
          .get(AppConstants.BASE_URL + AppConstants.SHIPPING_URI);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> placeOrder(Map<String, dynamic> orderData) async {
    try {
      final response = await dioClient.post(
          AppConstants.BASE_URL + AppConstants.ORDER_PLACE_URI,
          data: orderData);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponse> getTrackingInfo(String orderID) async {
    try {
      final response = await dioClient
          .get(AppConstants.BASE_URL + AppConstants.TRACKING_URI + orderID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
