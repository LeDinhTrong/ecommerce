...
class ProductRepo {
  ...
  Future<ApiResponse> getLatestProductList(
    ...
    try {
      final response = await dioClient.get(
        AppConstants.BASE_URL + AppConstants.LATEST_PRODUCTS_URI + offset,
  ...
  Future<ApiResponse> getBrandOrCategoryProductList(
    ...
    try {
      String uri;
      if (isBrand) {
        uri = '${AppConstants.BASE_URL + AppConstants.BRAND_PRODUCT_URI}$id';
      } else {
        uri = '${AppConstants.BASE_URL + AppConstants.CATEGORY_PRODUCT_URI}$id';
      }
  ...
  Future<ApiResponse> getRelatedProductList(
    ...
    try {
      final response = await dioClient.get(
        AppConstants.BASE_URL + AppConstants.RELATED_PRODUCT_URI + id,
        options: Options(headers: {AppConstants.LANG_KEY: languageCode}),
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
