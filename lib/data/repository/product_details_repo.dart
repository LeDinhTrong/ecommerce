...
class ProductDetailsRepo {
  ...
  Future<ApiResponse> getProduct(String productID, String languageCode) async {
    try {
      final response = await dioClient.get(
        AppConstants.BASE_URL + AppConstants.PRODUCT_DETAILS_URI + productID,
  ...
  Future<ApiResponse> getReviews(String productID) async {
    try {
      final response = await dioClient.get(
          AppConstants.BASE_URL + AppConstants.PRODUCT_REVIEW_URI + productID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getCount(String productID) async {
    try {
      final response = await dioClient
          .get(AppConstants.BASE_URL + AppConstants.COUNTER_URI + productID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> getSharableLink(String productID) async {
    try {
      final response = await dioClient.get(
          AppConstants.BASE_URL + AppConstants.SOCIAL_LINK_URI + productID);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
